...
async def _build_response_with_features(
    ...
    if tier == "tier2":
        x_values = [feat_dict.get(feat, np.nan) for feat in TIER2_FEATURES]
        X = np.array([x_values])
        result = engine.score_and_explain_tier2(X)[0]
    else:
        x_values = [feat_dict.get(feat, np.nan) for feat in TIER1_FEATURES]
        X = np.array([x_values])
        result = engine.score_and_explain_tier1(X)[0]

    # Filter out any features from unconsented sources (missing from feat_dict or NaN)
    result["shap_details"] = [
        f for f in result["shap_details"]
        if f["label"] in feat_dict and not (isinstance(feat_dict[f["label"]], (float, int)) and np.isnan(float(feat_dict[f["label"]])))
    ]
...
def _get_profile_features(
    ...
    consented_sources: list[str] | None = None,
) -> dict[str, Any]:
    import json
    sources = set(consented_sources) if consented_sources is not None else {
        "d1_bank", "d2_telecom", "d3_ecommerce", "d4_location", "d5_questionnaire", "d6_merchant_gst"
    }
    feat_dict = {}
    if "d1_bank" in sources:
        feat_dict.update(d1_bank_upi.generate(rng, profile))
    if "d2_telecom" in sources:
        feat_dict.update(d2_telecom.generate(rng, profile))
    if "d4_location" in sources:
        feat_dict.update(d4_location.generate(rng, profile))
    if "d5_questionnaire" in sources:
        feat_dict.update(d5_questionnaire.generate(rng, profile))
    if "d6_merchant_gst" in sources:
    ...
    if profile_data:
        if "bank_data" in profile_data and "d1_bank" in sources:
        ...
        if "telecom_data" in profile_data and "d2_telecom" in sources:
        ...
        if "ecommerce_data" in profile_data and "d3_ecommerce" in sources:
        ...
        if "location_data" in profile_data and "d4_location" in sources:
        ...
        if "questionnaire_data" in profile_data and "d5_questionnaire" in sources:
        ...
        if "gst_data" in profile_data and "d6_merchant_gst" in sources:
    ...
    if answers and "d5_questionnaire" in sources:
        try:
...
async def _build_response(
    ...
    feat_dict = None
    if getattr(settings, "use_sam_pipeline", False):
        try:
            import httpx
            async with httpx.AsyncClient(timeout=15.0) as client:
                sam_payload = {
                    "user_id": user_id,
                    "phone": phone,
                    "consented_sources": consented_sources or ["d1_bank", "d2_telecom", "d3_ecommerce", "d4_location", "d5_questionnaire", "d6_merchant_gst"],
                    "profile": profile,
                    "answers": answers,
                ...
                sam_resp = await client.post(settings.sam_api_url, json=sam_payload)
                if sam_resp.status_code == 200:
                    sam_data = sam_resp.json()
                    feat_dict = sam_data.get("features", {})
        except Exception as sam_err:
            print(f"SAM local pipeline unavailable, falling back to in-process: {sam_err}")

    if feat_dict is None:
        feat_dict = _get_profile_features(user_id, phone, rng, profile, answers, time_taken_ms, changes_count, consented_sources)
    ...
    sources_set = set(consented_sources) if consented_sources is not None else {"d3_ecommerce"}
    if "d3_ecommerce" in sources_set and not getattr(settings, "use_sam_pipeline", False):
...
async def post_score(body: ScoreRequest) -> ScoreResponse:
    engine = _get_engine()
    seed = int(hashlib.sha256(f"{body.user_id}:score_rng".encode("utf-8")).hexdigest(), 16) % (2**32)
    ...
    return await _build_response(
        ...
        consented_sources=body.consented_sources,
    )
